...
bool handle_mm_fault (struct pt_entry *ve);
...
process_execute (const char *file_name) 
{
  ...
  // oom 해결
  struct thread *child;
  for (struct list_elem *elem = list_begin(&(thread_current()->child_list)); elem != list_end(&thread_current()->child_list); elem = list_next(elem)) {
  ...
  }//만든 tid랑 같은 child 찾아서, 100% 찾음.

  sema_down(&(child->loading_sema));// 차일드 락 걸어. 걸려있으면 기다려.
                                  // 차일드 실행중이면 기다리는 거
...
start_process (void *file_name_)
{
  ...
  sema_up(&(thread_current()->loading_sema)); // 나 실행 끝났으니까 부모 실행해도 돼
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  for (struct list_elem *elem = list_begin(&(cur->child_list)); elem != list_end(&(cur->child_list)); elem = list_next(elem)) {
...
process_exit (void)
{
  ...
  // palloc_free_page(cur->thread_fd);???
  ...
  for(int i=2;i<MAX_FD;i++){
    ...
    if(cur->thread_fd[i] != NULL) {
      cnt++;
      file_close(cur->thread_fd[i]);
    }
  }//load 말고 여기서 닫아줘야 rox-child
  // print("------%d------\n",cnt); 닫은 fd 개수
  // vm_delete(&cur->vm);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      // ve->file = file;
      ve->file = file_reopen(file);//이유를모르겠어요 디버깅여기서터짐
      // strcpy(ve->file,filename,strlen(filename)+1);
      ve->offset = ofs;
      ve->is_loaded = false;
      ve->writable = writable;
      // struct pt_entry *pte = pt_create_entry (upage, BINARY, writable, false,
